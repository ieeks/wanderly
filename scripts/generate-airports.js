#!/usr/bin/env node
/**
 * Generates the AIRPORTS array for src/data/mockData.js
 *
 * Data sources (priority order):
 *   1. DE_OVERRIDES  — hand-curated German names for DACH + top destinations
 *   2. Wikidata      — German municipality labels via SPARQL (cached in wikidata-cities.json)
 *   3. CSV fallback  — English municipality name from airport-codes dataset
 *
 * Usage:
 *   node scripts/generate-airports.js                    # use cached Wikidata (or skip if no cache)
 *   node scripts/generate-airports.js --refresh-wikidata # re-fetch Wikidata SPARQL
 *
 * Output: prints updated AIRPORTS export to stdout → pipe into mockData.js via update-airports.js
 */
...
const CACHE_FILE = path.join(__dirname, 'wikidata-cities.json');
const CSV_URL    = 'https://raw.githubusercontent.com/datasets/airport-codes/master/data/airport-codes.csv';
const SPARQL_URL = 'https://query.wikidata.org/sparql';

// Hand-curated overrides — always win. German city + short airport name.
const DE_OVERRIDES = {
  // DACH
...
function fetchText(url, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get({ hostname: opts.hostname, path: opts.pathname + opts.search, headers: { 'User-Agent': 'wanderly-airport-generator/1.0', ...headers } }, res => {
...
const SPARQL = `
SELECT ?iata ?cityDE WHERE {
  ?airport wdt:P238 ?iata .
  ?airport wdt:P131 ?municipality .
  ?municipality rdfs:label ?cityDE .
  FILTER(LANG(?cityDE) = "de")
  FILTER(STRLEN(?iata) = 3)
}
`.trim();

async function fetchWikidata() {
  console.error('Fetching Wikidata SPARQL (this may take 20–40 s)…');
  const url = `${SPARQL_URL}?query=${encodeURIComponent(SPARQL)}&format=json`;
  const raw = await fetchText(url, { Accept: 'application/sparql-results+json' });
  const json = JSON.parse(raw);

  // Build iata → cityDE map; prefer shorter/simpler names when duplicates appear
  const map = {};
  for (const b of json.results.bindings) {
    const iata = b.iata?.value?.trim().toUpperCase();
    const city = b.cityDE?.value?.trim();
    if (!iata || iata.length !== 3 || !city) continue;
    if (!map[iata] || city.length < map[iata].length) {
      map[iata] = city;
    }
  }
  return map;
...
async function loadWikidata(refresh) {
  if (!refresh && fs.existsSync(CACHE_FILE)) {
    console.error(`Using cached Wikidata names (${CACHE_FILE})`);
    return JSON.parse(fs.readFileSync(CACHE_FILE, 'utf8'));
  }
  const map = await fetchWikidata();
  fs.writeFileSync(CACHE_FILE, JSON.stringify(map, null, 2));
  console.error(`Wikidata cache saved → ${CACHE_FILE} (${Object.keys(map).length} entries)`);
  return map;
}
...
async function main() {
  const refresh = process.argv.includes('--refresh-wikidata');
  const update  = process.argv.includes('--update');

  console.error('Fetching airport CSV…');
  const csv = await fetchCSV(CSV_URL);
  const rows = parseCSV(csv);
  const filtered = rows
    .filter(r => r.type === 'large_airport' && r.iata_code && r.iata_code.length === 3)
    .sort((a, b) => a.iata_code.localeCompare(b.iata_code));
  console.error(`CSV: ${filtered.length} large airports`);

  // Try to load Wikidata; non-fatal if it fails
  let wikiCities = {};
  try {
    wikiCities = await loadWikidata(refresh);
  } catch (e) {
    console.error(`Wikidata unavailable (${e.message}) — using CSV names as fallback`);
  }

  const overrideCount = { manual: 0, wikidata: 0, csv: 0 };
  const airports = [];
  ...
  for (const row of filtered) {
    ...
    // 2. Wikidata municipality label (German)
    if (wikiCities[iata]) {
      const csvName = (row.name || '').replace(/\s*International\s*Airport/i, '').replace(/\s*Airport/i, '').trim();
      airports.push({ iata, city: wikiCities[iata], name: csvName || wikiCities[iata] });
      overrideCount.wikidata++;
      continue;
    }

    // 3. CSV English fallback
    const city = toTitleCase(row.municipality || row.name || iata);
    const name = (row.name || '').replace(/\s*International\s*Airport/i, '').replace(/\s*Airport/i, '').trim();
    airports.push({ iata, city, name: name || city });
    overrideCount.csv++;
  }

  console.error(`Sources — manual: ${overrideCount.manual}, wikidata: ${overrideCount.wikidata}, csv: ${overrideCount.csv}`);

  // Format JS export block
  const maxCity = Math.max(...airports.map(a => a.city.length));
  const lines = airports.map(a => {
    const iataStr = `'${a.iata}'`.padEnd(5);
    const cityStr = `'${a.city.replace(/'/g, "\\'")}'`.padEnd(maxCity + 2);
    const nameStr = `'${a.name.replace(/'/g, "\\'")}'`;
    return `  { iata:${iataStr}, city:${cityStr}, name:${nameStr} },`;
  });
  const block = ['export const AIRPORTS = [', ...lines, '];'].join('\n');

  if (update) {
    // Patch mockData.js in-place
    const mockPath = path.join(__dirname, '..', 'src', 'data', 'mockData.js');
    const src = fs.readFileSync(mockPath, 'utf8');
    if (!/export const AIRPORTS = \[[\s\S]*?\];/.test(src)) {
      console.error('ERROR: AIRPORTS block not found in mockData.js — nothing written.');
      process.exit(1);
    }
    const patched = src.replace(/export const AIRPORTS = \[[\s\S]*?\];/, block);
    fs.writeFileSync(mockPath, patched);
    console.error(`mockData.js updated — ${airports.length} airports written.`);
  } else {
    console.log(block);
    console.error(`\nDone — ${airports.length} airports total. (Use --update to write directly to mockData.js)`);
  }
}
