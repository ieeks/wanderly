...
const WORKER_URL = import.meta.env.VITE_PARSER_URL;

export async function extractTripFromEmail(emlContent) {
  if (!WORKER_URL) throw new Error('Parser nicht konfiguriert (VITE_PARSER_URL fehlt)');

  const res = await fetch(WORKER_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content: emlContent }),
  });

  if (!res.ok) throw new Error(`Parser-Fehler: ${res.status}`);

  return buildTrip(await res.json());
}
